    ...
    <>
        <div className='flex justify-between pt-5 px-5 items-center gap-4'>
                        ...
                        <div className='flex gap-2'>
                            <button onClick={()=>setListView(true)} className={`border-2 border-black rounded-md p-1 ${listView ? 'bg-zinc-700' : ''}`}>
                            ...
                            <button onClick={()=>setListView(false)} className={`border-2 border-black rounded-md p-1 ${!listView ? 'bg-zinc-700' : ''}`}>
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-6 h-6">
